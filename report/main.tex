...
\begin{abstract}
\paragraph{English}
This paper examines the development of a simple compiler that makes use of several phases: scanner, parser, symbol collector, desugar, code generator, allocator and code emitter. The scanner and parser are based on Python Lex-Yacc.
...
\paragraph{Danish}
Dette papir undersøger udviklingen af en simpel compiler, der gør brug af flere faser: scanner, parser, symbol collector, desugar, code generator, allocator and code emiter. Scanneren og parseren er baseret på Python Lex-Yacc. 
...
Valg og overvejeler er gjort når det har været nødvendigt, hvorfor det vil blive indset i reprospekt, at få ting bør have blevet gjort anderledes. Compiler implementationen er baseret hovedsagligt på teori givet i føglende litteratur: \cite{ModernCompilerImplementation}, \cite{EnginneringACompiler} og SCIL kendt fra DM565. 
...
Running the algorithm on the input yields the color mapping (temporary: color) given below. The highest degree first heuristic would have colored the graph using only two colors.
...
The implemented language is named Panda, and resembles a subset of C. The implemented types are Boolean and Integer (actually both are integers). Constructions such as if-else statements, while-loops and for-loops have also been implemented. \SI{64}{bit} are used for all types of values. It had been well regarded with composite types, such as arrays, but unfortunately, it was not achieved in the time frame.
